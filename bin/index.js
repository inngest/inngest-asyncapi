...
var INNGEST_CONFIG_PATH = import_node_path.default.join(import_node_os.default.homedir(), ".config/inngest/state");
var getCredentials = () => {
  const file = import_node_fs.default.readFileSync(INNGEST_CONFIG_PATH, "utf8");
  const config = JSON.parse(file);
  const buff = Buffer.from(config.credentials, "base64");
  return buff.toString("ascii");
};
var request = async (query = "", variables = {}) => {
...
var getProductionWorkspaceID = async () => {
  ...
  return res.workspaces.find((w) => w.name === "default" && w.test === false).id;
};
var getEventCollections = async (workspaceID) => {
  const res = await request(
...
var run = async () => {
  ...
  import_node_fs2.default.writeFileSync(`./${FILENAME}`, js_yaml_default.dump(spec));
  console.log(`\u{1F389} Successfully generated AsyncAPI spec!
